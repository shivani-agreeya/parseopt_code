...
object ReportDataParserSingleTable extends Serializable with LazyLogging {
  ...
  def parseLogData(spark: SparkSession,jobId: Long,fileName: String,processType: String,testId: Long,tableSuffix: String) = {
    ...
    logger.info(s"File processed in new single table script $tableSuffix")
    ...
    val SINGLE_TABLE_NAME: String ="FinalLogRecord_DMAT"
    val dmatDataFrameString=s""" 
      select * from (select fileName,dmTimeStamp, testId, insertedDate,
      ${QcommReportQuery.getQuery},
      ${NASReportQuery.getQuery},
      ${ASNReportQuery.getQuery},
      ${Qcomm2ReportQuery.getQuery},
      last_value(lteeNBId, true) over(partition by filename, dmTimeStamp order by dmTimeStamp desc rows between unbounded preceding and unbounded following) as lteeNBId,
      row_number() over(partition by filename, dmTimeStamp order by dmTimeStamp desc) as rn
      from (select *
      from dmat_logs.$SINGLE_TABLE_NAME$tableSuffix where filename in ($fileName) and testId in ($testId) and (inbldUserMark != 'true' or inbldUserMark != 'false'))) where rn = 1 
      """
    //logger.info(s"dmatDataframe >>>>> $dmatDataFrameString")  
    var dmatDataframe=spark.sql(dmatDataFrameString) 
    
    dmatDataframe.createOrReplaceTempView("dmatDataframe")
    dmatDataframe = dmatDataframe.persist(StorageLevel.MEMORY_AND_DISK)
    ...
    val winOperator = Window.partitionBy($"fileName").orderBy($"dmTimeStamp".asc).
    rowsBetween(Window.unboundedPreceding, -1)
    
    val lteDf: DataFrame = dmatDataframe.select($"fileName", $"dmTimeStamp", $"lteCellID", $"lteeNBId")
      .withColumn("LteCellID_deriv", coalesce($"lteCellID", last($"lteCellID", true).over(winOperator)))
      .withColumn("LteeNBId_deriv", coalesce($"lteeNBId", last($"lteeNBId", true).over(winOperator)))
      .drop("lteCellID", "lteeNBId")
      
    val puschTxPowerDf = dmatDataframe.select($"fileName", $"dmTimeStamp", $"pUSCHTxPower")
      .withColumn("PuschTxPower_deriv", coalesce($"pUSCHTxPower", last($"pUSCHTxPower", true).over(winOperator)))
      .drop("pUSCHTxPower") 
      
   finalOperatorKpiDf = finalOperatorKpiDf
      .join(lteDf, Seq("fileName", "dmTimeStamp"), "full_outer")
      .join(puschTxPowerDf, Seq("fileName", "dmTimeStamp"), "full_outer")
   
   val rlcTputDFTemp = spark.sql("select * from (select fileName,dmTimeStamp, rlcDlTotalBytes, rlcDlTimeStamp, rlcUlTotalBytes, rlcUlTimeStamp " +
      "from (select * from dmatDataframe where  filename in (" + fileName + ") and testId in (" + testId + ")))")

   val windPdcpRlc = Window.partitionBy($"fileName").orderBy($"dmTimeStamp".asc)

   val rlcTputDF = rlcTputDFTemp
      ...
      .drop("rlcDlTotalBytes", "rlcDlTimeStamp", "rlcUlTotalBytes", "rlcUlTimeStamp")
    val pdcpRlcTputDFCount= rlcTputDFTemp.count() 
   var finalJoinedTables: DataFrame = spark.createDataFrame(spark.sparkContext.emptyRDD[Row], schema)
    var joinedTablesTemp = finalOperatorKpiDf
      .join(rlcTputDF, Seq("fileName", "dmTimeStamp"), "full_outer")

    finalJoinedTables = dmatDataframe.join(joinedTablesTemp, Seq("fileName", "dmTimeStamp"), "left_outer")
    dmatDataframe.unpersist()
    
     val dmatMilliSecQuery: String = s"""select * from (select fileName,dmTimeStamp, testId, insertedDate,  ${B192ReportQuery.getReportQuery} , ${B193ReportQuery.getReportColumns}, ${ASNReportQuery.getAsnReportColumns},
      ...
      from (select *
      from dmat_logs.$SINGLE_TABLE_NAME$tableSuffix where  filename in ( $fileName) and testId in ($testId)))"""

    var dmatMillSecDataframe = spark.sql(dmatMilliSecQuery)
    dmatMillSecDataframe.createOrReplaceTempView("dmatMillSecDataframe")

    dmatMillSecDataframe = dmatMillSecDataframe.persist(StorageLevel.MEMORY_AND_DISK)

    
    val dmatB192_B193 = spark.sql(s"""select fileName,dmTimeStamp, ${B192ReportQuery.getQuery} ,
       ${B193ReportQuery.getQuery} from (select *  
      from dmatMillSecDataframe sort by fileName, dmTimeStamp desc) group by fileName, dmTimeStamp""")
    ...
    val flatten = udf((xs: WrappedArray[WrappedArray[WrappedArray[Int]]]) => xs.flatten.flatten.distinct.mkString(","))
    val flatten1 = udf((xs: WrappedArray[WrappedArray[Int]]) => xs.flatten.distinct.mkString(","))

    var dmatB192_B193NeighboringPciDist: DataFrame = spark.createDataFrame(spark.sparkContext.emptyRDD[Row], schema)
   
    // b192: collected_rsrp:done,collected_rsrq: Done,collected_rssi,collected_pci:done
    
    //b193: collectedB193_pci: Done,collectedB193_servingCell:Done,
    //collectedB193_rsrp:done,version_B193,
    //SubPacketSize_B193,collectedB193_snir0: Done,collectedB193_snir1,collectedB193_rsrq,
    ...
    dmatB192_B193NeighboringPciDist = dmatB192_B193merged.select($"fileName", $"collected_pci", $"collectedB193_pci", $"collectedQcomm_lteeNBId").groupBy("fileName").
    agg(flatten(collect_list($"collected_pci")).alias("NeighboringPCI"), flatten(collect_list($"collectedB193_pci")).
    alias("ServingPCI"), flatten1(collect_list($"collectedQcomm_lteeNBId")).alias("Serving_lteeNBId")) 
    
    var joinedMillSecDf = spark.createDataFrame(spark.sparkContext.emptyRDD[Row], schema)  
    if (dmatB192_B193merged != null)
      joinedMillSecDf = joinedMillSecDf.join(dmatB192_B193merged, Seq("fileName", "dmTimeStamp"), "full_outer")
    
    finalJoinedTables = finalJoinedTables.join(joinedMillSecDf, Seq("fileName", "dmTimeStamp"), "full_outer")
    finalJoinedTables.createOrReplaceTempView("finalJoinedTables")

    dmatMillSecDataframe.unpersist()  
    
    var joinedAllTables: DataFrame = finalJoinedTables
    val numRows=joinedAllTables.count
    joinedAllTables.show()
    logger.info(s"the number of rows in the file $fileName is $numRows")
   /* val qcomm1Query=spark.sql(s"""select * from (select fileName,dmTimeStamp, 
    ...
    val b193Query = spark.sql(s"""select fileName,dmTimeStamp,
    ...
    PhysicalCellID_B193,ServingCellIndex_B193, InstMeasuredRSRP, version,SubPacketSize,
  ...
  //joinedQuery.show()
*/   
  val reportRDD=joinedAllTables.mapPartitions(iter => {
	iter.map(row => {
	  //fileName
    ...
    val earfcn_b193: String = if (!row.isNullAt(row.fieldIndex("EARFCN"))) row.getString(row.fieldIndex("EARFCN")) else null
	 ...
	 ReportKpis(fileName=fileName, ltePccRsrp=ltePccRsrp, lTEPccSinr=ltePccSinr, ltePccRsrq=ltePccRsrq, ltePccRssi=ltePccRssi,
	           lteBand=ltePccBandInd, 
                      ...
                      lteTaReqCnt=lteTaReqCnt, lteTaRej=LTETaRej, lteAttCmpCnt=lteAttCmpCnt,ltePccTputDL = -9999999.0D, ltePccCqiCw0=lteAttCmpCnt,
